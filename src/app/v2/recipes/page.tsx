...
export default function RecipesPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (meRes?.ok) {
        ...
        if (me.isGuest) { window.location.replace('/v2'); return }
  ...
  return (
    <V2Shell
      ...
      title="Recipes"
      description="Build recipes, connect ingredients to recent invoice prices, and keep batch and per-unit costs current."
      actions={<button className="bp-btn bp-btn--primary" onClick={startNewRecipe}>New recipe</button>}
      wide
    >
