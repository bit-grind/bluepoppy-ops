...
function HolidayDropdown({ onSelect, disabled }: { onSelect: (q: string, display: string) => void, disabled: boolean }) {
  ...
  function select(name: string) {
    setOpen(false)
    onSelect(
      `What were the total gross sales on ${name}, what was the weather like, and what were the top 10 selling food items (exclude coffees, drinks, and beverages)? Present the food items using the standard Product Sales table format (Rank, Product Name, Quantity Sold, Sales $, Cost $, Gross Profit %). Show gross sales only, not net. Be brief and factual — no summary or recommendations.`,
      name
    )
  }
...
export default function AskPage() {
  ...
  return (
    <V2Shell
      ...
      title="Ask AI"
      description="Ask plain-language questions across sales, products, suppliers, bills, weather, and trading dates."
    >
